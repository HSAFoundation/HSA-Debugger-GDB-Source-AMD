...
//==============================================================================
#ifndef __DBGINFOCONSUMERIMPL_H
#define __DBGINFOCONSUMERIMPL_H
...
void DbgInfoConsumerImpl<AddrType, LineType, VarLocationType>::addLeafMemberNamesToVector(const ConsumedVariableInfo& rVarInfo, const std::string& namesBase, std::vector<std::string>& io_variableNames) const
{
    ...
    if ((ConsumedVariableInfo::DID_IND_POINTER == rVarInfo.m_varIndirection) || (ConsumedVariableInfo::DID_IND_ARRAY == rVarInfo.m_varIndirection) /* || (DV_POINTER_ENCODING == rVarInfo.m_varEncoding) */)
    {
                ...
                if ((stackFrameDepth == -1) || (stackFrameDepth == currentStackDepth) || (currentStackDepth == totalStackDepth))
                {
                    ...
                    for (int i = 0; i < numberOfVariables; i++)
                    {
                        ...
                        if (pCurrentVariable != nullptr)
                        {
                            ...
                            if ((pCurrentVariable->VarValueType() == ConsumedVariableInfo::DID_VAR_CONSTANT_VALUE) || ((pCurrentVariable->m_highVariablePC >= addr) && (pCurrentVariable->m_lowVariablePC <= addr)))
                            {
                                if (finalMembers)
                                {
