...
//==============================================================================
#ifndef __DBGINFODATA_H
#define __DBGINFODATA_H
...
public:
    /// Whether the type of var Constant or Variable / Parameter:
    enum VariableValueType
    {
        DID_VAR_CONSTANT_VALUE,
        DID_VAR_VARIABLE_VALUE,
        DID_VAR_PARAMETER_VALUE,
    };

    /// The variable indirection:
    enum VariableIndirection
    {
        DID_IND_DIRECT,
        DID_IND_POINTER,
        DID_IND_REFERENCE,
    ...
    /// Accessor to get the Variable Value Type
    VariableValueType VarValueType() const {return m_varValueType;}
    ...
    VariableIndirection m_varIndirection;    ///< The indirection Type
...
private:
    VariableValueType m_varValueType;        ///< Const, Var or Param, can only be set via SetConstantValue() and ClearConstantValue()
};
...
VariableInfo<AddrType, VarLocationType>::VariableInfo()
    : m_varName(),
      ...
      m_varSize(0),
      m_varEncoding(0),
      m_varIndirection(DID_IND_DIRECT),
      ...
      m_isOutParam(false),
      m_brigOffset(0),
      m_varValueType(DID_VAR_VARIABLE_VALUE)
{
}
...
void VariableInfo<AddrType, VarLocationType>::SetConstantValue(HwDbgUInt64 bufferSize, unsigned char* pDataBuffer)
{
    ...
    m_varValueType = DID_VAR_CONSTANT_VALUE;
};
...
{
    if (DID_VAR_CONSTANT_VALUE == m_varValueType)
    {
...
VariableInfo<AddrType, VarLocationType>& VariableInfo<AddrType, VarLocationType>::operator=(const FullVariableInfo& other)
{
    ...
    m_varValueType = other.m_varValueType;
    ...
    while (pCurrentScope != nullptr)
    {
        ...
        for (int i = 0; i < numberOfVars; ++i)
        {
            ...
            if (pCurrentVar != nullptr)
            {
                ...
                {
                    // If const:
                    if (FullVariableInfo::DID_VAR_CONSTANT_VALUE == pCurrentVar->VarValueType())
                    {
                        foundVar = true;
        ...
        if (!foundVar)
        {
            ...
            // Uri, 23/7/13 - This generates incorrect results when the boundary address is one of the addresses mapped.
            // Disabling until I can rethink the algorithm:
            /*
            if (!shouldTraverse)
            {
...
void CodeScope<AddrType, LineType, VarLocationType>::IntersectVariablesInScope()
{
    ...
    for (int i = 0; i < numberOfVariables; ++i)
    {
        ...
        if (pCurrentVariable != nullptr)
        {
            ...
            if (FullVariableInfo::DID_VAR_CONSTANT_VALUE == pCurrentVariable->VarValueType())
            {
                continue;
            }
            ...
            for (int j = (i + 1); j < numberOfVariables; j++)
            {
                ...
                {
                    // Do not need to intersect Constants:
                    if (FullVariableInfo::DID_VAR_CONSTANT_VALUE == pOtherVariable->VarValueType())
                    {
                        continue;
                    }
