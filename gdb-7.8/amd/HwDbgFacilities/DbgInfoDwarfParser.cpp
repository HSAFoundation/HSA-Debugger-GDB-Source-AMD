...
// Uri, 16/11/10: libelf and winnt.h define this macro as two different things.
...
//@{
/// \def Uri, 18/11/10: our implementation of DWARF is missing some values in the header file, added them here:
...
//@}
//@{
/// \def Uri, 1/2/12: private AMD values for DWARF:
#define DW_AT_AMDIL_address_space 0x3ff1
#define DW_AT_AMDIL_resource 0x3ff2
//@}

//@{
/// \def gilad, 2/26/13: private AMD values for DWARFed until <hsa_dwarf.h> included:
...
//@{
/// \def Uri, 5/9/13: private AMD values for DWARF DW_AT_address_class until <hsa_dwarf.h> is updated:
...
void DbgInfoDwarfParser::FillChildren(Dwarf_Die programDIE, const std::string& firstSourceFileRealPath, Dwarf_Debug pDwarf, DwarfCodeScope& o_scope)
{
    ...
    while (goOn)
    {
        ...
        if (rc == DW_DLV_OK)
        {
            ...
            switch (currentChildTag)
            {
                ...
                {
                    // This is a variable/const/parameter, create an object: (note: if this is a DID_VAR_CONSTANT_VALUE we will need to get the value and call SetConstantValue())
                    DwarfVariableValueType varValueType = GetVariableValueTypeFromTAG(currentChildTag);
                    DwarfVariableInfo* pVariable = new DwarfVariableInfo;

                    // Need to Initialize the location:
                    if (varValueType != DwarfVariableInfo::DID_VAR_CONSTANT_VALUE)
                    {
                    ...
                    HWDBG_ASSERT(varValueType != DwarfVariableInfo::DID_VAR_CONSTANT_VALUE || numberOfAdditionalLocations == 0);

                    if (varValueType != DwarfVariableInfo::DID_VAR_CONSTANT_VALUE || numberOfAdditionalLocations == 0)
                    {
    ...
    // Uri, 27/1/14: There's a bug in our libDWARF implementation, where dwarf_child() sometimes returns the error code instead
...
/// \return
/// ---------------------------------------------------------------------------
DbgInfoDwarfParser::DwarfVariableValueType DbgInfoDwarfParser::GetVariableValueTypeFromTAG(int dwarfTAG)
{
    DwarfVariableValueType retVal = DwarfVariableInfo::DID_VAR_VARIABLE_VALUE;

    switch (dwarfTAG)
    {
        case DW_TAG_formal_parameter:
            retVal = DwarfVariableInfo::DID_VAR_PARAMETER_VALUE;
            break;

        case DW_TAG_variable:
            retVal = DwarfVariableInfo::DID_VAR_VARIABLE_VALUE;
            break;

        case DW_TAG_constant:
            retVal = DwarfVariableInfo::DID_VAR_CONSTANT_VALUE;
            break;
...
void DbgInfoDwarfParser::FillInlinedFunctionData(Dwarf_Die programDIE, const std::string& firstSourceFileRealPath, Dwarf_Debug pDwarf, DwarfCodeScope& o_scope)
{
    ...
    if ((rc == DW_DLV_OK) && (callFileNumber > 0))
    {
        ...
        // so we do not need the temp file path from the compiler:
        if (nullptr != o_scope.m_inlineInfo.m_inlinedAt.m_fullPath)
        {
            o_scope.m_inlineInfo.m_inlinedAt.m_fullPath->clear();
        }
        else
        {
            o_scope.m_inlineInfo.m_inlinedAt.m_fullPath = new std::string;
        }

        if ((callFileNumber == 1) && (!firstSourceFileRealPath.empty()))
        {
            // Set the real path as input by the user:
            *o_scope.m_inlineInfo.m_inlinedAt.m_fullPath = firstSourceFileRealPath;
        }
            ...
            if ((rc == DW_DLV_OK) && (cuDIE != nullptr))
            {
                ...
                if ((rc == DW_DLV_OK) && (pSourceFilesAsCharArrays != nullptr) && (numberOfSourceFiles > 0))
                {
                    if (numberOfSourceFiles >= (Dwarf_Signed)callFileNumber)
                    {
                        ...
                        *o_scope.m_inlineInfo.m_inlinedAt.m_fullPath = pSourceFilesAsCharArrays[callFileNumber - 1];
                    }
        ...
        while (!scopesToCheck.empty())
        {
            ...
            if (pCurrentScope != nullptr)
            {
                ...
                for (int i = 0; i < numberOfVariables; i++)
                {
                    ...
                    {
                        if (pCurrentVariable->VarValueType() != DwarfVariableInfo::DID_VAR_CONSTANT_VALUE)
                        {
...
void DbgInfoDwarfParser::FillTypeNameAndDetailsFromTypeDIE(Dwarf_Die typeDIE, Dwarf_Debug pDwarf, bool expandIndirectMembers, bool isRegisterParamter, DwarfVariableInfo& o_variable)
{
    ...
    o_variable.m_varEncoding = DV_UNKNOWN_ENCODING;
    o_variable.m_varIndirection = DwarfVariableInfo::DID_IND_DIRECT;
    bool goOn = true;
    ...
    bool foundIndirectionDetail = false; // TO_DO: support multiple level indirection (__global int* __local pointerArray[10];) when the OpenCL spec supports it.
        ...
        if (rc == DW_DLV_OK)
        {
                ...
                if (DW_DLV_OK == rc)
                {
                    ...
                    switch (addressClassAsDWUnsigned)
                    {
                        case Amd_HSA_Private:
                            o_variable.m_varIndirectionDetail = VID_PRIVATE_POINTER;
                            break;

                        case Amd_HSA_Global:
                            o_variable.m_varIndirectionDetail = VID_GLOBAL_POINTER;
                            break;

                        case Amd_HSA_Readonly:
                            o_variable.m_varIndirectionDetail = VID_CONSTANT_POINTER;
                            break;

                        case Amd_HSA_Group:
                            o_variable.m_varIndirectionDetail = VID_LOCAL_POINTER;
                            break;

                        /*                  case Amd_HSA_Region:
                                                o_variable.m_varIndirectionDetail = VID_REGION_POINTER;
                                                break;                                                  */
                        default:
                            // Unexpected Value!
                            HWDBG_ASSERT(false);
            ...
            switch (currentTypeTag)
            {
                case DW_TAG_array_type:
                    ...
                    if (DwarfVariableInfo::DID_IND_DIRECT == o_variable.m_varIndirection)
                    {
                        o_variable.m_varIndirection = DwarfVariableInfo::DID_IND_ARRAY;
                    }
                ...
                case DW_TAG_pointer_type:
                    ...
                    if (DwarfVariableInfo::DID_IND_DIRECT == o_variable.m_varIndirection)
                    {
                        o_variable.m_varIndirection = DwarfVariableInfo::DID_IND_POINTER;
                    }
                ...
                case DW_TAG_reference_type:
                    ...
                    if (DwarfVariableInfo::DID_IND_DIRECT == o_variable.m_varIndirection)
                    {
                        o_variable.m_varIndirection = DwarfVariableInfo::DID_IND_REFERENCE;
                    }
                ...
                case DW_TAG_union_type:
                    ...
                    if (o_variable.m_varEncoding == DV_UNKNOWN_ENCODING)
                    {
                        o_variable.m_varEncoding = DV_NO_ENCODING;
                    }
    ...
    if ((o_variable.m_varIndirection ==  DwarfVariableInfo::DID_IND_POINTER || o_variable.m_varIndirection == DwarfVariableInfo::DID_IND_ARRAY) && (o_variable.m_varEncoding == DV_UNKNOWN_ENCODING))
    {
        o_variable.m_varEncoding = DV_POINTER_ENCODING;
    }
    ...
    if ((o_variable.m_varEncoding == DV_POINTER_ENCODING) && (!expandIndirectMembers))
    {
        hasMembers = false;
    }
    ...
    if (o_variable.m_varEncoding == DV_UNKNOWN_ENCODING || o_variable.m_varIndirection != DwarfVariableInfo::DID_IND_DIRECT)
    {
    ...
    if (hasMembers)
    {
        ...
        if (rcCh == DW_DLV_OK)
        {
            ...
            while (currentChild != nullptr)
            {
                ...
                bool isConst = (o_variable.VarValueType() == DwarfVariableInfo::DID_VAR_CONSTANT_VALUE);
...
void DbgInfoDwarfParser::FillVariableWithInformationFromDIE(Dwarf_Die variableDIE, Dwarf_Debug pDwarf, bool isMember, DwarfVariableInfo& o_variableData, std::vector<DwarfVariableLocation>& o_variableAdditionalLocations)
{
    ...
    // Check if const:
    bool isConst = (DwarfVariableInfo::DID_VAR_CONSTANT_VALUE == o_variableData.VarValueType());
    ...
    if ((!isConst) && (locType == DwarfVariableLocation::LOC_REG_REGISTER) && (o_variableData.VarValueType() == DwarfVariableInfo::DID_VAR_PARAMETER_VALUE))
    {
    ...
    if ((isRegisterParameter) && (o_variableData.m_varEncoding == DV_NO_ENCODING))
    {
...
void DbgInfoDwarfParser::FillVarIndirectionDetails(Dwarf_Die variableDIE, DwarfVariableInfo& o_variable)
{
    ...
    {
        case DwarfVariableInfo::DID_IND_POINTER:
        {
            ...
            if (rc == DW_DLV_OK)
            {
                switch (addressSpaceAsDwarfUnsigned)
                {
                    ...
                    default:
                        ...
                        o_variable.m_varIndirectionDetail = VID_UNKNOWN_POINTER;
                        break;
                }
            }
        ...
        case DwarfVariableInfo::DID_IND_ARRAY:
            break;

        case DwarfVariableInfo::DID_IND_REFERENCE:
            break;
...
void DbgInfoDwarfParser::FillVarEncoding(Dwarf_Die variableDIE, DwarfVariableInfo& o_variable)
{
    ...
    if (rcFm == DW_DLV_OK)
    {
        switch (typeEncodingAsDWARFUnsigned)
        {
            ...
            case DW_ATE_boolean:
                o_variable.m_varEncoding = DV_BOOL_ENCODING;
                break;

            case DW_ATE_float:
                o_variable.m_varEncoding = DV_FLOAT_ENCODING;
                break;

            case DW_ATE_signed:
                o_variable.m_varEncoding = DV_INT_ENCODING;
                break;

            case DW_ATE_signed_char:
                o_variable.m_varEncoding = DV_CHAR_ENCODING;
                break;

            case DW_ATE_unsigned:
                o_variable.m_varEncoding = DV_UINT_ENCODING;
                break;

            case DW_ATE_unsigned_char:
                o_variable.m_varEncoding = DV_UCHAR_ENCODING;
                break;
    ...
    void* userData)
{
    ...
    else
    {
        ...
        // TO_DO: Uri - replace this with the actual BRIG statement table:
...
/// -----------------------------------------------------------------------------------------------
KernelBinary::KernelBinary(void* pBinaryData, size_t binarySize) : m_pBinaryData(nullptr), m_binarySize(0)
{
...
/// \param[in]          binarySize
/// -----------------------------------------------------------------------------------------------
void KernelBinary::setBinary(void* pBinaryData, size_t binarySize)
{
    if (nullptr != m_pBinaryData)
    {
        delete [](char*)m_pBinaryData;
    ...
    {
        m_pBinaryData = new unsigned char[m_binarySize];
        ::memcpy(m_pBinaryData, pBinaryData, m_binarySize);
    }
}
...
bool KernelBinary::getSubBufferAsBinary(size_t offset, size_t size, KernelBinary& o_bufferAsBinary) const
{
    ...
    {
        // Copy the data:
        o_bufferAsBinary.setBinary((void*)((size_t)m_pBinaryData + offset), size);
        retVal = true;
    }
