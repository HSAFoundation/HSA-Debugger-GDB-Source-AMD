...
//==============================================================================
#ifndef __DBGINFOLINES_H
#define __DBGINFOLINES_H
...
public:
    ...
    //@}
    const std::string& fullPath() const { return *m_fullPath;}; /// Accessor
    std::string* m_fullPath; ///< The full path of the file
...
bool LineNumberMapping<AddrType, LineType>::AddLineMapping(const LineType& line, const AddrType& addr)
{
    ...
    if (isNewAddr)
    {
        ...
        if (isNewLine)
        {
            ...
            std::vector<AddrType> addressesMappedToLine;
