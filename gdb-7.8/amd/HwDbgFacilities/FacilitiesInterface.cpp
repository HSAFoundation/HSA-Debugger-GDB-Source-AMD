...
{
    // TODO: Uri, May 14th, 2015 - Currently, we do not handle members of HLL variables. If we want to support such code, we would need to attach the member names to the
...
HwDbgInfo_debug hwdbginfo_init_with_hsa_1_0_binary(void* bin, size_t bin_size, HwDbgInfo_err* err)
{
    // Validate input:
    ...
    for (size_t i = 0; i < secCount; ++i)
    {
        ...
        {
            // Found a code object!
            // TO_DO: support multiple code objects
    ...
    if (!retVal)
    {
        // Uri, May 17th, 2015 - the final May format has the DWARF information in the main HSA file. If it's there, use it now:
    ...
    {
        /*
        // Uri, May 13th, 2015: the .hsatext section seems to be the BRIG code, as it is
...
HwDbgInfo_debug hwdbginfo_init_with_two_binaries(void* hl_bin, size_t hl_bin_size, void* ll_bin, size_t ll_bin_size, HwDbgInfo_err* err)
{
    ...
    if (rcHLLM)
    {
        ...
        {
            const std::string* currentFileName = hl_fileLocs[i].m_fullPath;

            if (nullptr != currentFileName && !currentFileName->empty())
            {
...
HwDbgInfo_err hwdbginfo_code_location_details(HwDbgInfo_code_location loc, HwDbgInfo_linenum* line_num, size_t buf_len, char* file_name, size_t* file_name_len)
{
    ...
    {
        HWDBGFAC_INTERFACE_OUTPUT_STRING((*pLoc->m_fullPath), file_name, buf_len, file_name_len, err);
    }
...
HwDbgInfo_err hwdbginfo_frame_context_details(HwDbgInfo_frame_context frm, HwDbgInfo_addr* pc, HwDbgInfo_addr* fp, HwDbgInfo_addr* mp, HwDbgInfo_code_location* loc, size_t buf_len, char* func_name, size_t* func_name_len)
{
    ...
    HwDbgInfo_err err = HWDBGINFO_E_SUCCESS;
    const std::string* pFullPath = pFrame->m_sourceLocation.m_fullPath;
    size_t pathLen = (nullptr != pFullPath) ? pFullPath->length() : 0;
    HWDBGFAC_INTERFACE_VALIDATE_OUTPUT_ARRAY(pathLen + 1, func_name, buf_len, err);
    HWDBGFAC_INTERFACE_CHECKRETURN(err);

    // Output code location:
    HWDBGFAC_INTERFACE_CSTR_FROM_STDSTRP(pPath, pFullPath);

    if (nullptr != loc)
    {
        *loc = hwdbginfo_make_code_location(pPath, pFrame->m_sourceLocation.m_lineNum);
    }
...
HwDbgInfo_err hwdbginfo_addr_to_line(HwDbgInfo_debug dbg, HwDbgInfo_addr addr, HwDbgInfo_code_location* loc)
{
    ...
    // Output the location:
    HWDBGFAC_INTERFACE_CSTR_FROM_STDSTRP(pPath, matchedLine.m_fullPath);
    *loc = hwdbginfo_make_code_location(pPath, matchedLine.m_lineNum);
...
HwDbgInfo_err hwdbginfo_nearest_mapped_line(HwDbgInfo_debug dbg, HwDbgInfo_code_location base_line, HwDbgInfo_code_location* line)
{
    ...
    if (nullptr == pBaseLine->m_fullPath || pBaseLine->m_fullPath->empty())
    {
        wasEmptyPath = true;
        ...
        {
            delete pBaseLine->m_fullPath;
            pBaseLine->m_fullPath = new std::string(pDbg->m_firstMappedFileName);
        }
    }
    ...
    HWDBGFAC_INTERFACE_CSTR_FROM_STDSTRP(pPath, matchedLine.m_fullPath);
    *line = hwdbginfo_make_code_location(pPath, matchedLine.m_lineNum);
...
HwDbgInfo_err hwdbginfo_variable_data(HwDbgInfo_variable var, size_t name_buf_len, char* var_name, size_t* var_name_len, size_t type_name_buf_len, char* type_name, size_t* type_name_len, size_t* var_size, HwDbgInfo_encoding* encoding, bool* is_constant, bool* is_output)
{
    ...
    // TO_DO: replace with a switch block?
    if (nullptr != encoding)
    {
    ...
    {
        *is_constant = (DbgInfoTwoLevelConsumer::LowLvlVariableInfo::DID_VAR_CONSTANT_VALUE == pVar->VarValueType());
    }

    if (nullptr != is_output)
    {
        *is_output = pVar->m_isOutParam;
    }
...
HwDbgInfo_err hwdbginfo_variable_location(HwDbgInfo_variable var, HwDbgInfo_locreg* reg_type, unsigned int* reg_num, bool* deref_value, unsigned int* offset, unsigned int* resource, unsigned int* isa_memory_region, unsigned int* piece_offset, unsigned int* piece_size, int* const_add)
{
    ...
    if (DbgInfoTwoLevelConsumer::LowLvlVariableInfo::DID_VAR_CONSTANT_VALUE == pVar->VarValueType())
    {
