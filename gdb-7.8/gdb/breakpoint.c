 ...
 * \file  breakpoint.c
 * \brief Edits for hsail breakpoints  */
...
/* Hsail communication and breakpoint facilities includes */
			       ...
			       int allflag)
{
  ...
  /* Print the HSAIL specific condition on a new line*/
  if ((b->type == bp_hsail) &&
      hsail_breakpoint_is_real_condition(&(b->hsail_bp_request->condition)))
    {
      ui_out_text (uiout, "\n\t");
...
install_breakpoint (int internal, struct breakpoint *b, int update_gll)
{
  ...
  /* We dont want to print the hsail breakpoint information just yet
   * We will print it at the end of the create_breakpoint_hsail function
   * when we have parsed the request
   * */
  if (!internal && b->type != bp_hsail)
    mention (b);
...
/* #define HSAIL_PC_BP */
   ...
   * b->ops->print_mention = hsail_bkpt_print_mention;
   * This needs the implementation of print_mention
   * The problem is that we cannot update the ops structure here, we need a new place
   *
   * Breakpoint can be  added within global list since we are choosing to set it up like
   * a raw breakpoint now. We should be able to delete it fine, but I am not sure if
   * there any negatives of setting raw bkpts
   */
  ...
  {
    hsail_enqueue_create_breakpoint_packet(b->hsail_pc, b->number);
  }
    ...
    case HSAIL_BP_TYPE_SOURCE_LOCATION:
      {
         ...
         * We use the same command since we do hsail:LINE_NUM
         */
  ...
  /*Print the breakpoint info in the form of HSAIL breakpoint #No#, #Line#, #Condition */
  printf_filtered (_("HSAIL breakpoint %d ("), b->number);
  /* Print the line */
  hsail_breakpoint_print_location(b);
  ...
  printf_filtered (")");
  if (!is_hsail_linux_initialized() || !hsail_is_debug_facilities_loaded())
    {
      printf_filtered (_(" pending."));
    }
  printf_filtered ("\n");
...
{
  gdb_assert(NULL != bpt);
  /* Note: In this function we cant use the data in bpt->hsail_bp_request
   * to get breakpoint number or any other information since the breakpoint
   * may not have been resolved yet and bpt->hsail_bp_request will
   * be uninitialized
   * */
  if (bpt->type == bp_hsail)
    {
      if (is_hsail_linux_initialized())
        {
          hsail_enqueue_delete_breakpoint_packet(bpt->number);
        }
      else
        {
          hsail_enqueue_delete_breakpoint_request_buffer(bpt->number);
        }

      if (bpt->hsail_bp_request != NULL)
        {
          xfree(bpt->hsail_bp_request);
          bpt->hsail_bp_request = NULL;
        }
    }
  return 1;
}
/*
 * We need to be sure that resetting HSAIL breakpoint is only called once per run of the inferior
 * That way we don't send multiple packets to the agent
 */
void breakpoint_re_set_all_hsail(void)
{
  ...
  /* We can assert initialization hasnt happened since we are calling this early and once
   *
   * However, the hsail buffer will already be initialized since this
   * function is called in the "post inferior create" function */
  ...
  ALL_BREAKPOINTS (b)
  {
    ...
    if (b == NULL)
      {
        printf_filtered("NULL breakpoint found in ALL_BREAKPOINTS loop\n");
        continue;
      }
    ...
    if (b->type == bp_hsail)
      {
        ...
        if(b->hsail_bp_request == NULL)
          {
            printf_filtered("Reset HSAIL breakpoints in post-create-inferior\n");
            printf_filtered("HSAIL Breakpoint %d \t with a NULL Request\n",b->number);
          }

        gdb_assert(b->hsail_bp_request != NULL);
        switch (b->hsail_bp_request->type)
        {
          case HSAIL_BP_TYPE_KERNEL_FUNCTION:
            {
              hsail_breakpoint_set_from_kernel_name(b->hsail_bp_request);
              break;
            }
          case HSAIL_BP_TYPE_SOURCE_LOCATION:
            {
              hsail_breakpoint_set_from_line(b->hsail_bp_request);
              break;
            }
          case HSAIL_BP_TYPE_ANY_LOCATION:
            {
              hsail_breakpoint_set_any(b->hsail_bp_request);
              break;
            }
            break;
          case HSAIL_BP_TYPE_UNKNOWN:
            {
              /* An unknown type is reasonable at this stage, since the hsail
               * breakpoint request  belonging to 'b' has not yet been populated,
               *
               * This may be if we set HSAIL breakpoints before we start the inferior, the
               * b->hsail_bp_request has not been populated yet.
               *
               * The request will be in the breakpoint cache and will be copied to
               * b->hsail_bp_request when we resolve after we initialize hsail or
               * initialize debug facilities.
               * */
              /*printf_filtered("Unknown HSAIL breakpoints in post-create-inferior\n");*/
            }
            break;
          default:
            /* This should really not happen since we set the BP type to unknown
             * in create_breakpoint_hsail
             * */
            printf_filtered("Uninitialized  HSAIL breakpoints: breakpoint_re_set_all_hsail\n");
            gdb_assert(0);
            break;
        }
      }
...
breakpoint_auto_delete (bpstat bs)
{
  ...
  for (; bs; bs = bs->next)
	...
	&& bs->stop)
      {
        /* Dont do these things for HSAIL */
        if (bs->breakpoint_at->type != bp_hsail)
        {
...
{
  gdb_assert(NULL != b);
  /* \todo To handle the HSAIL case, we dont use this function.
   *
   * HSAIL breakpoints only need to be recreated when the inferior is restarted.
   * When the inferior starts, we simply push all the hsail breakpoints to the agent.
...
delete_breakpoint (struct breakpoint *bpt)
{
  ...
  /*
   * Hsail specific delete steps
   */
  hsail_rc = 0;
  hsail_rc = delete_breakpoint_hsail(bpt);
  if(hsail_rc == 0)
    {
      printf_filtered("HSAIL breakpoint may not have been deleted safely\n");
    }
...
disable_breakpoint (struct breakpoint *bpt)
{
  ...
  if (bpt->type == bp_hsail)
    {
      hsail_enqueue_disable_breakpoint_packet(bpt->number);
    }
}
...
#define BREAK_HSAIL_ARGS_HELP()\
"\n\
For HSA applications: hsail-gdb supports function breakpoints \n\
...
Kernel source breakpoints: Break execution when execution hits an HSAIL \n\
source line \n\
Usage:\n\
break hsail\t\t\t Break on every HSAIL dispatch \n\
break hsail:[Kernel_name]\t Break when kernel [Kernel_name] is about to \n\
\t\t\t\t begin execution\n\
break hsail:[Line_number]\t Break when execution hits line [Line_number] \n\
\t\t\t\t in temp_source\n\n\
"
...
Set breakpoint at specified line or function. \nThis command can be used for HSAIL breakpoints and CPU breakpoints\n"
BREAK_HSAIL_ARGS_HELP()
