...
/* Since we cannot change the parameters of quit_command without
 * changing everything in gdb's command handling, we use the
 * below static variables to cache arguments
 * */
static char *quit_command_backup_args;
static int quit_command_backup_from_tty;
...
quit_command (char *args, int from_tty)
{
  ...
  bool is_quit_delay_needed = false;

  quit_command_backup_args = args;
  quit_command_backup_from_tty = from_tty;

  query_if_trace_running (from_tty);

  if (!skip_hsa_kill)
    {
      is_quit_delay_needed = hsail_tdep_kill_all_waves(true);
      if (is_quit_delay_needed)
        {
 ...
 * We now call the regular quit command to finish up the remaining
 * quit functions
 * */
void
quit_command_complete(void)
{
  skip_hsa_kill = true;
  skip_confirmation_check = true;

  quit_command (quit_command_backup_args, quit_command_backup_from_tty);
...
list_command (char *arg, int from_tty)
{
  ...
  if (hsail_is_focus_device())
    {
      hsail_cmd_list_command(arg, from_tty);
    }
