/*
   HSAIL breakpoint functions and interfacing with HSAIL debug facilities
...
static bool hsail_breakpoint_parse_bp_command(const char* hsail_bp_str, HsailBreakpointRequest* bp_request)
{
    ...
    /* Allowed formats:
     * hsail / hsail:       Break on any HSAIL location.
     * hsail:ker_name       Break on entry to kernel named ker_name.
     * hsail:file_name:123  Break on the indicated line on the indicated file name.
     * hsail:123            Break on the indicated line in the currently debugged kernel's main source file. */
    /* Get the size: */
    ...
    /* If the new string is empty, this is an "any" breakpoint ("hsail"): */
...
#pragma warning ("This part of the code was not designed for windows targets, and will require special handling to support Windows-style full paths 'X:\...' !")
    ...
    if (-1 == first_colon_loc)
    {
        ...
        /* Output the request (hsail:kern_name): */
    ...
    if (NULL != first_non_num)
    {
        ...
        if (0 == ll)
        {
            ...
            /* Output the request (hsail:file_name:123 / hsail::123): */
                                       ...
                                       HsailBreakpointRequest* bp_request)
{
  ...
  if(!status)
    {
      printf("Could not parse HSAIL breakpoint condition string\n");
      return status;
    }
  ...
  if(!status)
    {
      printf("Could not parse the HSAIL breakpoint request.\n");
      printf("hsail-gdb will now parse \"%s\" as a host code breakpoint.\n", ip_hsail_bp_str);
      return status;
    }
  free(hsail_bp_str);
...
char* hsail_dbginfo_get_source_buffer(void)
{
  ...
  if (gs_hsail_source == NULL)
    {
      ui_out_text(uiout, "HSAIL source buffer not yet populated\n");
    }
                                            ...
                                            const HwDbgInfo_linenum line_num)
{
  ...
  /*
   * GDB's build gives a warning that tmpnam is unsafe since it is only valid for TMP_MAX
   * calls to the function.
   * However, on Linux, TMP_MAX is 238328 so we are fine. This is only called when user
   * creates a source breakpoint
   *
   * The problem with mkstemp which is recommended to be used here is that I dont know if
   * it can be opened for both read and write.
   *
   * Note: as per the man page, temp_file_name is a static buffer internally
   * and should not be 'freed'
   */
  temp_file_name = tmpnam(NULL);
  gdb_assert(temp_file_name != NULL);
  ...
  if(hsail_is_debug_facilities_loaded())
    {
      ...
      if (errOut != HWDBGINFO_E_SUCCESS)
        {
           ...
           * */
          ui_out_text(uiout, "[hsail-gdb]: The code object for the current dispatch does not contain debug information\n");
      ...
      if (errOut != HWDBGINFO_E_SUCCESS)
        {
          ui_out_text(uiout, "GDB: HwDbgFacilities get hsail text error\n");
        }
  ...
  if (0 == resolvedLoc && HWDBGINFO_E_NOTFOUND == err)
    {
      if (target_file_name == NULL)
        {
          printf_filtered("[hsail-gdb: No line %llu in temp_source]\n", line_num);
        }
      else
        {
          printf_filtered("[hsail-gdb: No line %llu in %s]\n", line_num, target_file_name);
        }
          ...
          if (p_bkpt != NULL)
            {
              if (p_bkpt->type != bp_hsail)
                {
                  printf_filtered("A non hsail type breakpoint was reported by the agent\n");
                }
...
void hsail_breakpoint_print_location(const struct breakpoint* p_bp)
{
      ...
      if (p_bp->hsail_bp_request->bp.kernel_func.func_name != NULL)
        {
          ...
          if(p_bp->hsail_bp_request->bp.kernel_func.func_name[0] == (char)'*' )
            {
              sprintf(hsail_any_bp_str, "Every HSAIL dispatch(%s)",
        ...
        {
          ui_out_field_string (uiout, "What", "Unknown HSAIL kernel breakpoint");
        }
      break;
    }
  case HSAIL_BP_TYPE_SOURCE_LOCATION:
  ...
  case HSAIL_BP_TYPE_ANY_LOCATION:
    ...
    break;
  default:
    ui_out_field_string (uiout, "addr", "Unknown HSAIL breakpoint type");
  }
