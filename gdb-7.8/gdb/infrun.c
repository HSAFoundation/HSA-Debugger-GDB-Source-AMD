/* Target-struct-independent code to start (run) and stop an inferior
   process. HSAIL specific stop printing
...
{
  annotate_signal ();
  /*\todo add some code that checks if signal == HSAIL signal
   * hsail_is_signal_from_agent(siggnal) == true checks for "Child status changed"
   * */
  if (hsail_is_focus_device() &&
      hsail_is_signal_from_agent(gdb_signal_to_string(siggnal)) == true)
    {
      ui_out_text (uiout, "Stopped on HSAIL breakpoint\n");
...
print_stop_event (struct target_waitstatus *ws)
{
  ...
  /* The behavior of this routine with respect to the source
     ...
     HSAIL specific changes have been done here so that we dont get the
     pthread_kill printing when we stop at  a HSAIL breakpoint.

     I am unsure of how to check for the signal received, we should print the
     stack frame if the signal is not hsail relevant.
     This function is called after print_signal_received_reason.

     See fb 11120
      */
  if (ws->kind == TARGET_WAITKIND_STOPPED)
    {
      if (hsail_is_signal_from_agent(
            gdb_signal_to_string(ws->value.sig)) && hsail_is_focus_device())
        {
          /* We dont have any stack frame to print for now*/
          printf("\n");
        }
      else
        {
