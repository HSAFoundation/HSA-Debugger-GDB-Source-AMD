...
/* HSAIL Specific */
struct sigaction sighsail_action;
...
/* Used to sync with the agent, once the fifo has been created the read end has be opened
 * and we can receive information from the agent
 * */

/*Used to sync with the agent, once the fifo has been created to check if the read end
 * can be opened*/
static int sighsail_pending_count = 0;

static void
sighsail_handler (int signo)
{
  int old_errno = errno;
  struct ui_out *uiout;
  uiout = current_uiout;

  ui_out_text(uiout, "[hsail-gdb: <SAW A SIGALRM - Used for HSAIL >]\n");
  ...
  /* Just update a single global memory location
   */
  if (signo == SIGALRM
      && linux_nat_event_pipe[0] != -1)
    {
      sighsail_pending_count += 1;
    }
		...
		int target_options)
{
  ...
  /* The new hsail initialization routine checks for the number of times
   * that SIGALRM is called.
   * The nat_wait function calls are triggered by the kill(SIGUSR1)
   * sent by the agent
   * */
  if (is_hsail_linux_initialized() != 1 && sighsail_pending_count == 1)
    {
      /* stg1: handles the fifo opening from GDB <== Agent*/
      hsail_linux_initialize_stg1();
    }
  if (is_hsail_linux_initialized() != 1 && sighsail_pending_count == 2)
    {
      /* stg2: handles the fifo opening from GDB ==> Agent*/
      hsail_linux_initialize_stg2();
      /* We can reset the pending count now*/
      sighsail_pending_count = 0;
    }
  ...
  /* We can add a check and assert that does:
   *
   * (is_hsail_linux_initialized() && sighsail_pending_count != 0)
   * \todo: However, we need to remove the SIGALRM notification in the agent
   * when we write the dbe binary to shmem.
   * Will be done once we port changes to gdb 7.8 agent
   */
...
static void
linux_nat_clear_pending_sighsail(void)
{
  sighsail_pending_count = 0;
}
...
linux_nat_close (struct target_ops *self)
{
  ...
  /* We need to clear the HSAIL pending signals in case any left over
     This can happen with RTs like C++AMP.
     In C++AMP, the Agent Loads multiple times which sends SIGALRMs to GDB.
     While the SIGALRMs are harmless since gdb does not try to init fifos
     multiple times in a process's lifetime, they are left pending and need to
     be cleared when the process ends */
  linux_nat_clear_pending_sighsail();

  hsail_free_command_buffer();
  /* The hsa agent will already be unloaded, we can close down the fifos safely now */
...
/* This function is only called once per run of the debugger process.
 * The code in gdb's autogenerated init.c function calls this function
 * */
void
...
Enables printf debugging output."),
  ...
  /* HSAIL Specific signal */
  /* Install a SIGALRM handler.  */
  sighsail_action.sa_handler = sighsail_handler;
  sigemptyset (&sighsail_action.sa_mask);
  sighsail_action.sa_flags = SA_RESTART;

  /* Make it the default.  */
  sigaction (SIGALRM, &sighsail_action, NULL);

  /* Make sure we don't block SIGHSAIL during a sigsuspend.  */
  sigprocmask (SIG_SETMASK, NULL, &suspend_mask);
