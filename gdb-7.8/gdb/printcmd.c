...
print_command_1 (const char *exp, int voidprint)
{
  ...
  struct cleanup *hsail_chain = NULL;
  char format = 0;
  struct value *val;
  ...
  if (exp && is_hsail_print(exp))
    {
      HsailPrintStatus printError = HSAIL_PRINT_SUCCESS;
      printFormat=(char*)malloc(10);
      val = hsail_print_expression(exp, printFormat, &format_size);
      printError = hsail_print_get_last_error();
      if (hsail_print_get_last_error() != HSAIL_PRINT_SUCCESS)
      {
        voidprint = 0;
        switch(printError)
        {
          case HSAIL_PRINT_WRONG_FORMAT:  error(_("Wrong hsail print format")); break;
          case HSAIL_PRINT_VAR_NOT_FOUND: error(_("Variable not found in current context")); break;
          case HSAIL_PRINT_NO_WAVES:      error(_("No hsail variables information available at start of kernel")); break;
        }
      }
      hsail_chain = make_cleanup (hsail_print_cleanup, val);

      /* if a format was defined in the hsail_print_expression but format was not provided by the user */
      if (strlen(printFormat) != 0 && 0 == fmt.format)
      {
         fmt = decode_format ((const char**)(&printFormat), last_format, 0);
         last_format = format = fmt.format;
      }
      /* force the type based on the formatting in case of a float */
      if (fmt.format == 'f' && 4 == format_size)
      {
        if (NULL != val)
        {
          struct type *type = check_typedef (value_type (val));
          if (NULL != type && NULL != type->main_type)
          {
            var_length = type->length;
            var_code = type->main_type->code;
            restore_var = true;
            type->length = 4;
            type->main_type->code = TYPE_CODE_FLT;
            type->main_type->name = "float";  
          }
        }
      }
    }
  ...
  if (voidprint || (val && value_type (val) &&
		    TYPE_CODE (value_type (val)) != TYPE_CODE_VOID))
    {
      ...
      if (restore_var)
      {
          ...
          if (NULL != type && NULL != type->main_type)
          {
            type->length = var_length;
            type->main_type->code = var_code;
          }
      }
  ...
  if (NULL != hsail_chain)
  {
    do_cleanups (hsail_chain);  
  }
  /* it appears that somewhere this is released in gdb need to verify that */
  /*  if (NULL != printFormat)
  {
    free(printFormat);
  }*/
...
#define PRINT_HSAIL_HELP()\
"When debugging an HSAIL kernel, HSAIL registers and variables can be printed.\n"\
"Usage:\n"\
"print hsail:[variable]\t Print value of [variable] for the focus work-item\n\n"\
...
Print value of expression EXP.\nCommand can be used to print HSAIL registers, variables and host expressions\n\
"\
PRINT_HSAIL_HELP()\
"\
For host expressions, print value of expression EXP.\n\
