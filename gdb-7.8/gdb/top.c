/*
   ...
   Also includes HSAIL cleanup functions
...
static void detect_hsail_gdb_environment(void)
{
  char* env_var = getenv("ENABLE_HSAIL_GDB");

  if (env_var == NULL)
    {
      printf_filtered("\n\nGDB has detected that it is not being run using the hsail-gdb.\n");
      printf_filtered("Debugging HSAIL dispatches may not work\n");
      printf_filtered("Please run hsail-gdb instead.\n\n\n");
      exit(1);
    }
}
...
and \"show warranty\" for details.\n");
  ...
  /* Following is the HSAIL build version of the gdb binary
     todo: any AMD copyrights? */

  fprintf_filtered (stream, "AMD hsail-gdb %s\n", hsail_gdb_version);
  detect_hsail_gdb_environment();
